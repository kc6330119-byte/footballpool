...
const submitted=z.object({Bryan:z.boolean(),Ed:z.boolean(),Mike:z.boolean(),Kevin:z.boolean()});
export const weekSchema=z.object({submitted:submitted.optional(),number:z.number().int().min(1).max(18),games:z.array(game).max(50),totalPoints:totals,recordedTotals:totals,winner:z.union([z.literal(''),z.enum(players)]),earnings:z.number().min(0).max(10000),note:z.string().max(500),dues,teamEarnings:z.record(z.number().min(0).max(10000)),actualTotal:point,locked:z.boolean(),deadlineLocal:z.string().refine(s=>Number.isFinite(deadlineUTC(s)))}).refine(w=>new Set(w.games.map(g=>g.id)).size===w.games.length);
export const picksSchema=z.object({week:z.number().int().min(1).max(18),revision:z.number().int().min(0),submit:z.boolean().default(false),picks:z.record(z.string().max(100)),totalPoints:point});
