...
export async function POST(request:Request){if(!sameOrigin(request))return NextResponse.json({error:'Request origin is not allowed.'},{status:403});try{const a=await access();if(!a.user||!a.player)return NextResponse.json({error:'Your sign-in has not been assigned a player.'},{status:403});const v=picksSchema.safeParse(await request.json());if(!v.success)return NextResponse.json({error:'Check your picks and total-points prediction.'},{status:400});const {pool}=await readPool();const w=pool.weeks[v.data.week-1];if(isClosed(w)||revealed(w))return NextResponse.json({error:'This week is locked.'},{status:409});const p=a.player;
 ...
 if(v.data.submit&&!completeEntry(w,p))return NextResponse.json({error:'Choose a team for every game and enter your total-points prediction before submitting.'},{status:400});
 w.submitted={...submissions(w),[p]:v.data.submit};
