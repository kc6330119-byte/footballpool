...
try{
 ...
 assert.equal(deliveries.length,1);assert.deepEqual(deliveries[0].bcc,['ed@example.test','kevin@example.test']);assert.equal(deliveries[0].to,'undisclosed-recipients:;');assert.equal(deliveries[0].from.address,'kevin@example.test');assert.ok(deliveries[0].html&&deliveries[0].text);
